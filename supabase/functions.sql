...
begin
  if v_method <> 'POST' or v_path is null
     or (v_path not like '%/rpc/ingresar_estudiante'
         and v_path not like '%/rpc/completar_perfil_docente') then
    return;
  end if;
...
-- reparar las dos URL de una comparación de videos sin alterar entregas.
...
begin
  ...
  if exists (
    ...
    where unidad_id = p_unidad_id and lower(btrim(titulo)) = lower(v_titulo)
  ) then
  ...
  insert into public.actividades (
    unidad_id, tipo_id, titulo, instrucciones, aprendizaje_esperado, video_url, contenido, orden
  ) values (
...
begin
  if not public.es_docente_activo() then raise exception 'Se requiere una cuenta docente confirmada.'; end if;
  if p_actividad_id is null or p_fecha is null then raise exception 'La actividad y su fecha de apertura son obligatorias.'; end if;
  if coalesce(cardinality(p_grupo_ids), 0) = 0 then raise exception 'Selecciona al menos un grupo para abrir la actividad.'; end if;
  if cardinality(p_grupo_ids) <> (select count(distinct id) from unnest(p_grupo_ids) as ids(id)) then raise exception 'La lista de grupos contiene duplicados.'; end if;

  select a.unidad_id, a.titulo into v_unidad_id, v_titulo
    from public.actividades a where a.id = p_actividad_id;
  if not found then raise exception 'La actividad no existe.'; end if;

  if exists (
    select 1 from unnest(p_grupo_ids) as seleccion(grupo_id)
      left join public.grupos g on g.id = seleccion.grupo_id and g.docente_id = (select auth.uid())
     where g.id is null
  ) then raise exception 'Solo puedes programar la apertura para tus propios grupos.'; end if;

  delete from public.eventos e
   where e.actividad_id = p_actividad_id and e.tipo = 'apertura_actividad'
     and e.docente_id = (select auth.uid())
     and e.grupo_id in (select g.id from public.grupos g where g.docente_id = (select auth.uid()))
     and not (e.grupo_id = any(p_grupo_ids));

  insert into public.eventos (docente_id, grupo_id, unidad_id, titulo, tipo, fecha, actividad_id)
  select (select auth.uid()), g.id, v_unidad_id, v_titulo, 'apertura_actividad', p_fecha, p_actividad_id
    from public.grupos g where g.id = any(p_grupo_ids) and g.docente_id = (select auth.uid())
...
begin
  v_actividad_id := public.crear_actividad_docente(
    p_unidad_id, p_tipo_id, p_titulo, p_instrucciones,
    p_aprendizaje_esperado, p_video_url, p_contenido
  );
...
begin
  ...
  if not found then
    raise exception 'La actividad no es válida.';
  end if;
  ...
  loop
    ...
    if found then
      ...
      if v_entrega.respuesta is not null
        ...
        if v_intentos_texto ~ '^[1-9][0-9]*$' then
      ...
      if v_intentos_previos >= v_max_intentos then
        if v_max_intentos = 2 then
          raise exception 'Ya usaste los 2 intentos de esta actividad.'
...
-- Estas dos funciones solo son accesibles por el servidor con service_role.
-- Mantienen muy corta la transacción: Gmail se llama fuera de Postgres y el
-- único bloqueo es la actualización atómica de la fila por reporte.
...
-- Seguimiento de reportes: la persona reportante puede confirmar que vio una
-- respuesta sin obtener permisos para modificar la atención administrativa.
create or replace function public.proteger_reporte_atencion()
returns trigger language plpgsql security definer set search_path = public
as $$
begin
