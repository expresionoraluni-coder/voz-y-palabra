...
begin
  ...
  if not v_es_activacion and v_credencial !~ '^[0-9]{4}$' then
    raise exception 'Escribe tu NIP de 4 dígitos o el código de acceso de 16 caracteres.';
  end if;
  ...
  select e.id, e.nombre, e.boleta, e.nip_hash, e.activo, e.auth_user_id, e.debe_cambiar_nip,
         e.activacion_hash, e.activacion_expira_en, e.intentos_fallidos, e.bloqueado_hasta into v_estudiante
  ...
  if v_estudiante.debe_cambiar_nip then
    ...
    update public.estudiantes
       set auth_user_id = auth.uid(), activacion_usada_en = coalesce(activacion_usada_en, now()),
           intentos_fallidos = 0, bloqueado_hasta = null
     where public.estudiantes.id = v_estudiante.id;
    return query select v_estudiante.id, v_estudiante.nombre, v_grupo.id, v_grupo.nombre, true, null::text;
    return;
  end if;
  if v_es_activacion or v_estudiante.nip_hash is null
     or extensions.crypt(v_credencial, v_estudiante.nip_hash) <> v_estudiante.nip_hash then
...
begin
  ...
  for v_item in select * from jsonb_array_elements(p_estudiantes) loop
    ...
    v_accesos := v_accesos || jsonb_build_array(jsonb_build_object(
      'id', v_estudiante_id,
      'nombre', v_nombre,
...
begin
  ...
  select id, nip_hash, debe_cambiar_nip, activacion_hash, activacion_expira_en,
         intentos_fallidos, bloqueado_hasta into v_estudiante
    from public.estudiantes
  ...
  if not coalesce(v_credencial_valida, false) then
    ...
    perform pg_sleep(0.5); return 'Tu NIP o código de acceso no es correcto.';
  end if;
  update public.estudiantes
...
begin
  ...
  update public.estudiantes
     set nip_hash = null,
         ...
         activacion_generada_en = now(),
         activacion_expira_en = now() + interval '30 days',
         activacion_usada_en = null
...
-- actividad que ya tiene entregas. El video de apoyo y las URL de una
-- comparación de videos siguen siendo editables porque no alteran la clave.
