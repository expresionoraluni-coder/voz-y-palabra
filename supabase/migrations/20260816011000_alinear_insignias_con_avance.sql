-- Las insignias de avance deben usar la misma regla que la ruta del estudiante:
-- mínimo de 70 puntos o tres intentos en actividades automáticas.
...
begin
  ...
  if v_total_reflexiones >= 1 then
    ...
    select v_estudiante, id from public.insignias where nombre = 'Primera reflexión' on conflict do nothing;
  end if;
  if v_total_reflexiones >= 3 then
    insert into public.insignias_otorgadas (estudiante_id, insignia_id)
    select v_estudiante, id from public.insignias where nombre = 'Mente reflexiva' on conflict do nothing;
  end if;
  ...
  if v_total_actividades > 0 and v_total_hechas = v_total_actividades then
    insert into public.insignias_otorgadas (estudiante_id, insignia_id)
    select v_estudiante, id from public.insignias where nombre = 'Voz y Palabra completo' on conflict do nothing;
  end if;
  if v_unidades_con_ambas_confianzas >= 1 then
    insert into public.insignias_otorgadas (estudiante_id, insignia_id)
    select v_estudiante, id from public.insignias where nombre = 'Autoconocimiento' on conflict do nothing;
  end if;
