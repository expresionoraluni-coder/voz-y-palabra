...
La migración `20260910040000_corregir_insignias_y_endurecer_docentes.sql` corrige la columna usada por las insignias, limita el bloqueo de orientaciones a la entrega, conserva las variantes alternativas y retira grants DML heredados del perfil docente. La migración `20260910040100_retirar_rpc_publico_invitacion.sql` elimina el RPC público de validación previa; el trigger de `auth.users` conserva la validación atómica de la invitación. La migración `20260910040200_restaurar_permisos_pre_request.sql` conserva el wrapper sin parámetros y le devuelve únicamente los permisos que PostgREST necesita para ejecutar `db_pre_request` bajo los roles `anon` y `authenticated`.
