-- Evita reportes innecesarios y conserva el contexto real de la pantalla.
-- La orientación rápida se resuelve en el cliente; solo se registra si la
-- persona todavía necesita ayuda.
...
alter table public.reportes
  ...
  check (categoria in ('acceso', 'actividad', 'avance', 'video', 'carga', 'contenido', 'orientacion', 'otro'));
...
begin
  ...
  if p_actividad_id is not null and not exists (select 1 from public.actividades a where a.id = p_actividad_id and (p_unidad_id is null or a.unidad_id = p_unidad_id)) then raise exception 'La actividad del reporte no es válida.'; end if;
