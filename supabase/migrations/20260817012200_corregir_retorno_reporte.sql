-- Corrige la referencia al identificador de salida, que comparte nombre
-- con la columna id de la tabla reportes.
...
as $$
declare v_existente uuid; v_prioridad text; v_contexto jsonb := coalesce(p_contexto, '{}'::jsonb);
begin
  ...
  if p_categoria not in ('acceso', 'actividad', 'avance', 'video', 'carga', 'contenido', 'otro') then raise exception 'La categoría no es válida.'; end if;
  ...
  select r.id into v_existente from public.reportes r where r.reportante_id = (select auth.uid()) and r.categoria = p_categoria and r.estado in ('recibido','en_revision','necesita_informacion') and r.created_at >= now() - interval '24 hours' order by r.created_at desc limit 1;
  ...
  values((select auth.uid()),p_reportante_tipo,p_estudiante_id,p_docente_id,p_grupo_id,p_unidad_id,p_actividad_id,p_categoria,trim(p_descripcion),v_prioridad,p_ruta,v_contexto)
