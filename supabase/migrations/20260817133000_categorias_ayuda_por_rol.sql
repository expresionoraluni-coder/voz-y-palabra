-- Separa las solicitudes de ayuda de estudiantes y docentes.
-- Se conservan las categorías anteriores para no invalidar casos históricos.

alter table public.reportes drop constraint if exists reportes_categoria_check;
alter table public.reportes
  add constraint reportes_categoria_check
  check (categoria in (
...
create or replace function public.registrar_reporte(
  p_reportante_tipo text, p_estudiante_id uuid, p_docente_id uuid, p_grupo_id uuid,
  p_unidad_id uuid, p_actividad_id uuid, p_categoria text, p_descripcion text,
  p_ruta text, p_contexto jsonb
)
...
begin
  ...
  select r.id into v_existente from public.reportes r
    where r.reportante_id = (select auth.uid()) and r.categoria = p_categoria
...
revoke all on function public.registrar_reporte(text, uuid, uuid, uuid, uuid, uuid, text, text, text, jsonb) from public, anon;
