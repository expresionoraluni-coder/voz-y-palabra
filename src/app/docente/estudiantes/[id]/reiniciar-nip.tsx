...
export default function ReiniciarNip({ estudianteId, nombre }: { estudianteId: string; nombre: string }) {
  ...
  const hecho = Boolean(codigoAcceso);
  ...
  async function reiniciar() {
    ...
    const { data: codigoGenerado, error: rpcError } = await supabase.rpc("reiniciar_nip_estudiante", {
    ...
    if (typeof codigoGenerado !== "string" || !/^[0-9A-F]{16}$/.test(codigoGenerado)) {
      setError("No recibimos un código de acceso válido. Intenta de nuevo.");
      setCargando(false);
      return;
    }
    setCodigoAcceso(codigoGenerado);
    setCargando(false);
    router.refresh();
  }
  ...
  if (hecho) {
    return (
      <div className="flex flex-col gap-1 text-sm text-emerald-600 dark:text-emerald-400">
        <p>Código personal para {nombre.split(" ")[0]}: <strong className="font-mono tracking-wider">{codigoAcceso}</strong></p>
        <p>Compártelo de forma privada. Al entrar, tendrá que crear un NIP propio.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-2 rounded-xl bg-slate-50 px-4 py-3 dark:bg-slate-800/60">
      <p className="text-sm text-slate-700 dark:text-slate-300">
        ¿Restablecer el acceso de {nombre}? Su sesión actual se cerrará y se generará un código personal nuevo.
        Después tendrá que crear un NIP propio.
      </p>
