...
export default async function Portafolio() {
  ...
  const secciones = (unidades ?? []).map((u) => {
    ...
    return { u, confInicio, confCierre, actividadesCompletadas };
  });
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-2xl flex-col gap-8 px-6 py-10 print:px-0">
      ...
      {secciones.map(({ u, confInicio, confCierre, actividadesCompletadas }) => {
              ...
              {actividadesCompletadas.map((a) => {
                ...
                return (
                  <Card key={a.id} className="p-4">
                    ...
                    {reflexion?.texto && (
                      <p className="mt-3 flex gap-1.5 border-t border-slate-100 pt-2.5 text-sm italic text-slate-500 dark:border-slate-800 dark:text-slate-500">
